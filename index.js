...
class webpackBuildLogPlugin{
  ...
  createLog(compiler){
    ...
    compiler.hooks.printFileSizesAfterBuildHook.tap('Listen4Myplugin', (FileSizesAfterBuild) => {
          FileSizesAfterBuild.then((previousFileSizes)=>{
            ...
            const logFile = `${this.options.path||compilation.options.context||"log"}/${this.options.filename||"compilation-detail.md"}`;
            ...
            ws.write(buffer, 'utf8',(err, buffer)=>{
              console.log(`\n`);
              console.log(chalk.blueBright(`Packing logs in this -> ${this.options.path||compilation.options.context}/${this.options.filename||"compilation-detail.md"}`));
              console.log(`\n`);
  ...
  getAssets(assetsParams,root,sizes,buildFolder){
    ...
    let {
      ...
      console.log(boxen(this.makeRow(
